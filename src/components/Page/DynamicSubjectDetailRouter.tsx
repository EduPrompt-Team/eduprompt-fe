  ...
  useEffect(() => {
    ...
    async function loadReviews() {
      try {
        ...
        // Load reviews, average rating, and count
        ...
        const [reviewsData, avgRating, count] = await Promise.all([
          reviewService.getReviewsByStorageId(selectedPrompt.storageId).catch((e: any) => {
            if (e?.response?.status === 404) {
              console.log('[DynamicRouter] No reviews found for storageId:', selectedPrompt.storageId);
              return [];
            }
            throw e;
          }),
          ...
          reviewService.getReviewCount(selectedPrompt.storageId).catch((e: any) => {
            if (e?.response?.status === 404) return 0;
            console.warn('[DynamicRouter] Failed to get review count:', e);
            return 0;
          }),
        ])
        ...
        <div className="grid lg:grid-cols-2 gap-8">
          ...
          {selectedPrompt && (
            <div className="space-y-6">
              ...
              <Button 
                onClick={() => navigate(`/grade${grade}/${subject}/detail/${chapter}/chat`)}
              ...
              <div className="bg-[#23233a] rounded-lg p-4 border border-[#2a2a44]">
                <div className="flex items-center space-x-3">
                  ...
                  ></div>
                  <div>
                    <h3 className="font-semibold text-sm">{selectedPrompt.templateName} Generator</h3>
